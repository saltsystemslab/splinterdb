...
static inline void
trunk_pivot_branch_tuple_counts(trunk_handle *spl, trunk_node *node, uint16 pivot_no, uint16 branch_no,
                                uint64 *num_tuples, uint64 *num_kv_bytes);
...
static inline bool32
trunk_branch_live_for_pivot(trunk_handle *spl, trunk_node *node, uint64 branch_no, uint16 pivot_no);
...
static inline void
trunk_zap_branch_range(trunk_handle *spl, trunk_branch *branch, key start_key, key end_key, page_type type);
...
platform_status
trunk_flush(trunk_handle *spl, trunk_node *parent, trunk_pivot_data *pdata, bool32 is_space_rec, uint64 *new_addr);
...
void trunk_split_index(trunk_handle *spl, trunk_node *parent, trunk_node *child, uint16 pivot_no,
                       trunk_compact_bundle_req *req);
...
static void
trunk_btree_skiperator_init(trunk_handle *spl, trunk_btree_skiperator *skip_itor, trunk_node *node, uint16 branch_idx,
...
#if SPLINTER_DEBUG
    platform_default_log("[Garbage collect] Getting bundle %d for node %lu\n", req->bundle_no, node.addr);
#endif
    trunk_bundle *bundle = trunk_get_bundle(spl, &node, bundle_no);
...
trunk_flush(trunk_handle *spl,
    ...
    trunk_pivot_add_bundle_tuple_counts(spl,
                                        ...
                                        req->input_pivot_kv_byte_count);
#if SPLINTER_DEBUG
    //platform_default_log("Incrementing refcount for child at address %lu\n", new_child.addr);
#endif
...
#if SPLINTER_DEBUG
    // platform_default_log("Compacting bundle for node %lu\n", req->addr);
#endif
    /*
    * 1. Acquire node read lock
    */
...
#if SPLINTER_DEBUG
        platform_default_log("[Split Index] Enqueuing compact bundle request for node %lu and bundle number %d\n", next_req->addr, next_req->bundle_no);
#endif
...
//platform_default_log("Query for key %s\n", (char *)slice_data(target.user_slice));
    ...
    char *target_str = (char *) slice_data(target.user_slice);
...
//   bool query_path_free = FALSE;
        ...
        if (node.hdr->num_aux_pivots != 0) {
            ...
            for (int i = 0; i < node.hdr->num_aux_pivots; i++) {
                ...
                if (start == 0) {
                    ...
                    start_key = key_create_from_slice(slice_create((size_t) strlen(start_str), start_str));
                }
                if (end == UINT64_MAX) {
                    end = POSITIVE_INFINITY_KEY;
                } else {
                    sprintf(end_str, "%ld", end);
                    end_key = key_create_from_slice(slice_create((size_t) end_str, end_str));
                }
                if (start_key.kind == NEGATIVE_INFINITY) {
                    int comp = slice_lex_cmp(target.user_slice, end_key.user_slice);
                    if (comp < 0) {
                        idx = i;
                        break;
                    }
                } else if (end_key.kind == POSITIVE_INFINITY) {
                    int comp = slice_lex_cmp(target.user_slice, start_key.user_slice);
                    if (comp >= 0) {
                        idx = i;
                        break;
                    }
                    ...
                    //! compare start first
                    int comp_start = slice_lex_cmp(target.user_slice, start_key.user_slice);
                    if (comp_start >= 0) {
                        //! our key is greater than or equal to start key
                        int comp_end = slice_lex_cmp(target.user_slice, end_key.user_slice);
                        if (comp_end < 0) {
                            idx = i;
                            break;
                        }
            ...
            if (idx != -1) {
                ...
                platform_assert(target_int >= start && target_int < end);
                if (spl->cfg.use_stats) {
    ...
    if (!should_continue) {
        ...
        if (use_p_star)
            platform_default_log("Key %lu queried for using P* pointer\n", target_int);
        platform_default_log("Key %lu queried for in the node [%lu, %lu)\n", target_int, lower_bound, upper_bound);
    }
...
trunk_print_pivots(platform_log_handle *log_handle,
    ...
    platform_log(log_handle,
                 "|--------------------------------------------------------------------------------------------------|\n");
    platform_log(log_handle,
                 "|         pivot key        |  child addr  |  filter addr | tuple count | kv bytes  |  srq  |  gen  |\n");
    platform_log(log_handle,
                 "|--------------------------|--------------|--------------|-------------|-----------|-------|-------|\n");
    // clang-format on
...
trunk_print_branches_and_bundles(platform_log_handle *log_handle,
    ...
    platform_log(log_handle,
                 "|--------------------------------------------------------------------------------------------------|\n");
    platform_log(log_handle,
                 "|                              BRANCHES AND [SUB]BUNDLES                                           |\n");
    platform_log(log_handle,
                 "|start_branch=%-2u end_branch=%-2u start_bundle=%-2u end_bundle=%-2u start_sb=%-2u end_sb=%-2u%-17s|\n",
                 start_branch,
                 end_branch,
                 start_bundle,
                 end_bundle,
    ...
    platform_log(log_handle,
                 "|--------------------------------------------------------------------------------------------------|\n");
    platform_log(log_handle,
                 "|   # |          point addr         | filter1 addr | filter2 addr | filter3 addr |                 |\n");
    platform_log(log_handle,
                 "|     |    pivot/bundle/subbundle   |  num tuples  |              |              |                 |\n");
    platform_log(log_handle,
                 "|-----|--------------|--------------|--------------|--------------|--------------|-----------------|\n");
    // clang-format on
...
trunk_print_insertion_stats(platform_log_handle *log_handle, trunk_handle *spl) {
    ...
    platform_log(log_handle,
                 "---------------------------------------------------------------------------------------------------------\n");
    platform_log(log_handle,
                 "  height | avg wait time (ns) | avg flush time (ns) | max flush time (ns) | full flushes | count flushes |\n");
    platform_log(log_handle,
                 "---------|--------------------|---------------------|---------------------|--------------|---------------|\n");
    ...
    platform_log(log_handle,
                 "------------------------------------------------------------------------------------------------------------------------------------------\n");
    platform_log(log_handle,
                 "  height | compactions | avg setup time (ns) | time / tuple (ns) | avg tuples | max tuples | max time (ns) | empty | aborted | discarded |\n");
    platform_log(log_handle,
                 "---------|-------------|---------------------|-------------------|------------|------------|---------------|-------|---------|-----------|\n");
    ...
    pack_time_per_tuple = global->root_compaction_tuples == 0 ? 0
                                                              : global->root_compaction_pack_time_ns /
                                                                global->root_compaction_tuples;
    platform_log(log_handle,
                 "    root | %11lu | %19lu | %17lu | %10lu | %10lu | %13lu | %5lu | %2lu | %2lu | %3lu | %3lu |\n",
    ...
    for (h = 1; h <= height; h++) {
        ...
        avg_compaction_tuples = global->compactions[rev_h] == 0 ? 0
                                                                : global->compaction_tuples[rev_h] /
                                                                  global->compactions[rev_h];
        pack_time_per_tuple = global->compaction_tuples[rev_h] == 0 ? 0
                                                                    : global->compaction_pack_time_ns[rev_h] /
                                                                      global->compaction_tuples[rev_h];
        platform_log(log_handle,
                     "%8u | %11lu | %19lu | %17lu | %10lu | %10lu | %13lu | %5lu | %2lu | %2lu | %3lu | %3lu |\n",
    ...
    platform_log(log_handle,
                 "--------------------------------------------------------------------------------------------------------------------------------\n");
    platform_log(log_handle,
                 "| leaf splits | avg leaves created | avg split time (ns) | max split time (ns) | single splits | ss avg tuples | ss max tuples |\n");
    platform_log(log_handle,
                 "--------------|--------------------|---------------------|---------------------|---------------|---------------|---------------|\n");
